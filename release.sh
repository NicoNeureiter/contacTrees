...
# Stamp the package ZIP with the version from version.xml (not the -SNAPSHOT pom version)
mvn clean package -DskipTests -Dbeast.pkg.version="${VERSION}"
echo ""
...
if [[ "${1:-}" == "--release" ]]; then
    ...
    <package name="${PKG_NAME}" version="${VERSION}"
        ...
        description="Phylogenetic model for language evolution with horizontal transfer">
